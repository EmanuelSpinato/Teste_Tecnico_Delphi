...
type
  ...
  protected
    ...
    procedure BeforePost; override;  // Implementação do BeforePost (se necessário)
...
begin
  // Aqui você pode adicionar a lógica necessária antes de editar os dados
  ShowMessage('Antes de editar a categoria!');
end;

procedure TfmCadastroCategorias.BeforePost;
begin
  // Lógica a ser executada antes de salvar o registro
  if Trim(edDescricao.Text) = '' then
  begin
    ShowMessage('A descrição não pode estar vazia!');
    Abort;  // Cancela a ação de salvar se a validação falhar
  end;

  // Caso precise, adicione mais validações ou modificações no registro antes do post
end;
